...
const Card = ({ children, className = '' }) => (
  <div className={`bg-card text-card-foreground p-6 rounded-lg border border-border shadow-sm ${className}`}>
    {children}
  </div>
);
...
function ReportDetailSection({ reportId, showAlert, navigateTo }) {
  ...
  const { summary, filteredAndSortedCompanies } = useMemo(() => {
    if (!reportData?.data) return { summary: {}, filteredAndSortedCompanies: [] };
    ...
    const nations = companies.reduce((acc, c) => {
        if (c.nation && c.nation !== 'Unknown') acc[c.nation] = (acc[c.nation] || 0) + 1;
        return acc;
    }, {});
    const peOwners = companies.reduce((acc, c) => {
        if (c.is_pe_owned && c.pe_owner_names) c.pe_owner_names.forEach(owner => acc[owner] = (acc[owner] || 0) + 1);
        return acc;
    }, {});
    ...
    const summaryData = {
      ...
      nationData: Object.entries(nations).map(([name, count]) => ({ name, count })).sort((a,b) => b.count - a.count).slice(0, 10),
      peOwnerData: Object.entries(peOwners).map(([name, size]) => ({ name, size }))
    };
    
    // --- Filtering and Sorting ---
    let processedCompanies = [...reportData.data];
    if (searchTerm) {
        processedCompanies = processedCompanies.filter(c => c.company_name.toLowerCase().includes(searchTerm.toLowerCase()));
    }
    ...
    return { summary: summaryData, filteredAndSortedCompanies: processedCompanies };
  }, [reportData, searchTerm, sortConfig]);

  if (isLoading) return <div className="text-center py-8"><i className="ph-fill ph-spinner-gap animate-spin text-4xl text-primary"></i></div>;
  ...
  return (
    <section className="space-y-8 animate-fade-in">
      {/* Header */}
      <div>
        <button className="text-primary font-semibold flex items-center mb-4" onClick={() => navigateTo('history')}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <StatCard title="PE Owned" value={`${summary.peOwned} (${summary.total > 0 ? ((summary.peOwned / summary.total) * 100).toFixed(1) : 0}%)`} icon="briefcase-metal" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-2">
            <h3 className="font-semibold mb-4 text-foreground">Ownership Distribution</h3>
            <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                    <Pie data={summary.ownershipData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={5} labelLine={false} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
        ...
        <Card className="lg:col-span-3">
            ...
            <ResponsiveContainer width="100%" height={300}>
                <BarChart data={summary.nationData} margin={{ top: 5, right: 20, left: 20, bottom: 5 }}>
                    <XAxis dataKey="name" />
                    <YAxis />
      ...
      {summary.peOwnerData?.length > 0 && (
        <Card>
            <h3 className="font-semibold mb-4 text-foreground">PE Firm Portfolio Size</h3>
            <ResponsiveContainer width="100%" height={400}>
                <Treemap
                    ...
                    stroke="#fff"
                    fill="#4f46e5"
                    content={<CustomizedContent colors={["#6366f1", "#818cf8", "#a5b4fc", "#c7d2fe"]} />}
                />
            </ResponsiveContainer>
        </Card>
      )}
      ...
      <Card>
        <h3 className="text-xl font-semibold mb-4 text-foreground">Detailed Company Data</h3>
        <input
            type="text"
          ...
          {filteredAndSortedCompanies.map((company) => (
            <div key={company.company_name} className="p-4 border rounded-lg bg-secondary/50">
                ...
                <div className="mt-2 text-sm space-y-2">
                    <p><strong className="font-semibold text-foreground">Status:</strong> {company.public_private}</p>
                    <p><strong className="font-semibold text-foreground">PE Owned:</strong> <span className={company.is_pe_owned ? 'text-green-600 font-bold' : ''}>{company.is_pe_owned ? 'Yes' : 'No'}</span></p>
                    <p><strong className="font-semibold text-foreground">Nation:</strong> {company.nation}</p>
                    {company.pe_owner_names.length > 0 && (
                        <p><strong className="font-semibold text-foreground">PE Owners:</strong> {company.pe_owner_names.join(', ')}</p>
                    )}
                    <p className="pt-2 border-t border-border/50"><strong className="font-semibold text-foreground">Ownership Details:</strong> <span className="text-muted-foreground">{company.ownership_structure}</span></p>
                </div>
            </div>
          ))}
        </div>
    ...
    return (
      <g>
        <rect
          ...
          height={height}
          style={{
            fill: colors[Math.floor((index / root.children.length) * colors.length)],
            stroke: '#fff',
            strokeWidth: 2,
          }}
        />
